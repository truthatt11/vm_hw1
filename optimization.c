...
static inline struct shadow_pair* hash_lookup(target_ulong guest_eip) {
...
static inline struct shadow_pair* hash_insert(target_ulong guest_eip, unsigned long* host_eip) {
    int index = guest_eip & (MAX_CALL_SLOT-1);
...
//    printf("(insert) guest_eip = %u, index = %d, host_eip = %p\n", guest_eip, index, host_eip);
    ...
    temp->shadow_slot = malloc(sizeof(unsigned long*));
...
{
    struct shadow_pair *host = hash_lookup(guest_eip);
    printf("(set) guest_eip = %u, host_eip = %p\n", guest_eip, host_eip);
    if(host != NULL)
        *host->shadow_slot = host_eip;
    else
...
void push_shack(CPUState *env, TCGv_ptr cpu_env, target_ulong next_eip)
{
    ...
    */
    ...
    TCGv_ptr shadow_ret_top = tcg_temp_new_ptr(), shadow_ret = tcg_temp_new_ptr();

    int label = gen_new_label(); 
    struct shadow_pair *host_eip = hash_lookup(next_eip);
    ...
    tcg_gen_ld_ptr (shadow_ret_top, cpu_env, offsetof(CPUState, shadow_ret_addr));
    ...
    tcg_gen_add_ptr(shadow_ret_top, shadow_ret_top, tcg_const_ptr(sizeof(unsigned long) * (SHACK_SIZE-1)));
    tcg_gen_st_ptr (shadow_ret_top, cpu_env, offsetof(CPUState, shadow_ret_addr_top));
 
    gen_set_label(label);

    tcg_gen_ld_ptr(shack_temp_top, cpu_env, offsetof(CPUState, shack_top));
    tcg_gen_ld_ptr(shadow_ret_top, cpu_env, offsetof(CPUState, shadow_ret_addr_top));
    tcg_gen_add_ptr(shack_temp_top, shack_temp_top, tcg_const_ptr(-sizeof(target_ulong)));
    tcg_gen_add_ptr(shadow_ret_top, shadow_ret_top, tcg_const_ptr(-sizeof(unsigned long)));
    // store guest_eip
    tcg_gen_st_ptr(tcg_const_ptr(next_eip), shack_temp_top, 0);
    tcg_gen_st_ptr(shack_temp_top, cpu_env, offsetof(CPUState, shack_top));
    // store host_eip
    tcg_gen_st_tl(tcg_const_tl((unsigned long)host_eip->shadow_slot), shadow_ret_top, 0);
    tcg_gen_st_ptr(shadow_ret_top, cpu_env, offsetof(CPUState, shadow_ret_addr_top));
	
    tcg_temp_free_ptr(shack_temp_top);
    tcg_temp_free_ptr(shack_temp_end);
    tcg_temp_free_ptr(shadow_ret);
    tcg_temp_free_ptr(shadow_ret_top);
...
{
    TCGv_ptr shack_top_ptr, shadow_ret_top_ptr, slot_ptr, host_eip;
    TCGv guest_eip;
    int elseLabel;

    shack_top_ptr = tcg_temp_new_ptr();
    shadow_ret_top_ptr = tcg_temp_new_ptr();
    slot_ptr = tcg_temp_new_ptr();
    guest_eip = tcg_temp_new();
    host_eip = tcg_temp_local_new_ptr();
    ...
    */
    ...
    tcg_gen_ld_ptr(shack_top_ptr, cpu_env, offsetof(CPUState, shack_top));
    tcg_gen_ld_ptr(slot_ptr, shadow_ret_top_ptr, 0);
    tcg_gen_ld_ptr(host_eip, slot_ptr, 0);
    ...
    tcg_gen_brcond_ptr(TCG_COND_EQ, host_eip, tcg_const_ptr(0), elseLabel);
